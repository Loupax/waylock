...
pub fn run(io: Io, gpa: mem.Allocator, options: Options) void {
    ...
    if (std.c.getuid() != std.c.geteuid()) {
        if (std.c.setgid(std.c.getgid()) != 0) fatal("failed to drop group privileges", .{});
        if (std.c.setuid(std.c.getuid()) != 0) fatal("failed to drop user privileges", .{});
    }
